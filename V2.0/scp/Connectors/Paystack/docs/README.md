...
Paystack payment service provider adapter for transaction initialization, verification, and webhook signature validation. Uses **stub responses** in `testing` and when `PAYSTACK_SECRET_KEY` is empty; calls Paystack HTTP API in other environments when the secret key is configured.
