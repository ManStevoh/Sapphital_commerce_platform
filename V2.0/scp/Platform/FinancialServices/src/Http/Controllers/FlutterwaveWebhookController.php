...
final class FlutterwaveWebhookController
{
    ...
    public function __invoke(Request $request): JsonResponse
    {
        ...
        if (! $this->webhookSignatures->verifyFlutterwave($payload, $signature)) {
        ...
        $event = $this->webhookSignatures->flutterwaveConnectorForPayload($payload)->handleWebhook($decoded);
        $payloadHash = hash('sha256', $payload);

        if ($event['reference'] !== ''
            && $this->webhookEvents->hasBeenProcessed('flutterwave', $event['event'], $event['reference'])) {
            return response()->json([
                'received' => true,
                'duplicate' => true,
            ]);
        ...
        if ($event['event'] === 'charge.completed' && $event['reference'] !== '') {
            $handled = false;
            ...
            if ($handled) {
                $this->webhookEvents->record('flutterwave', $event['event'], $event['reference'], $payloadHash);
            }
        }
